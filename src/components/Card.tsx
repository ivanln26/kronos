...
const Card: NextPage<Props> = (
  ...
  const setStateColor = (state: string) => {
    for (let s in stateColors) {
      if (s === state) {
        console.log(
          Object.values(stateColors)[Object.keys(stateColors).indexOf(s)],
        );
  ...
  return (
    <div className="flex h-20 select-none">
      ...
      <div
        className={"flex grow flex-col justify-center rounded-r-xl px-2 border-l-8 border-blue-500 bg-primary-99 bg-gradient-to-r from-primary-40/[.05] to-primary-40/[.05] " +
          setStateColor(state) +
          " dark:bg-neutral-10 dark:from-primary-80/[.05] dark:to-primary-80/[.05]"}
      >
        <h1>{course}</h1>
        <h1>{classroom}</h1>
        <h1>{teacher}</h1>
