...
const LectureDetail: NextPage<LectureDetailProps> = ({ }) => {
  ...
  return (
    <>
      <Navbar />
      <div id="alert-1" className={"flex p-4 mb-4 text-blue-800 rounded-lg bg-blue-50 dark:bg-gray-800 dark:text-blue-400 mt-2 mx-3" + (showAlert ? " visible" : " hidden")} role="alert">
        ...
        <button onClick={() => setShowAlert(false)} type="button" className="ml-auto -mx-1.5 -my-1.5 bg-blue-50 text-blue-500 rounded-lg focus:ring-2 focus:ring-blue-400 p-1.5 hover:bg-blue-200 inline-flex h-8 w-8 dark:bg-gray-800 dark:text-blue-400 dark:hover:bg-gray-700" data-dismiss-target="#alert-1" aria-label="Close">
            ...
            : data && (
              <section className="prose lg:prose-2xl dark:prose-invert min-w-full px-2 py-6 my-4 rounded bg-gradient-to-r bg-primary-99 from-primary-40/[.05] to-primary-40/[.05] dark:bg-neutral-10 dark:from-primary-80/[.05] dark:to-primary-80/[.05]">
                <div className="flex justify-between">
                  <div>
                    <Link href={"/"}>
                      <Image src={myIcon} alt="back" />
                    </Link>
                  </div>
                  {session?.user.role === "professor" && session?.user.id === data.teacherId.toString() && (
                    <div>
